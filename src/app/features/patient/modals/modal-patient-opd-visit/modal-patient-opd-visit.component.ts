...
export class ModalPatientOpdVisitComponent {
  ...
  async getDiag() {
    this.loading = true;
    try {
      const response: any = await this.emrService.getOpdDiag(this.hospcode, this.seq, this.zoneKey);
      this.loading = false;
  ...
  async getDrug() {
    this.loading = true;
    try {
      const response: any = await this.emrService.getOpdDrug(this.hospcode, this.seq, this.zoneKey);
      this.loading = false;
  ...
  async getLab() {
    this.loading = true;
    try {
      const response: any = await this.emrService.getOpdLab(this.hospcode, this.seq, this.zoneKey);
      this.loading = false;
  ...
  async getOpdInfo() {
    this.loading = true;
    try {
      const response: any = await this.emrService.getOpdInfo(this.hospcode, this.seq, this.zoneKey);
      this.loading = false;
